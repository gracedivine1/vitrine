    ...
    function ProduitDialogController($timeout, $scope, $stateParams, $uibModalInstance, entity, ProduitService, CatProduitService) {
        ...
        function save() {
            ...
            if (vm.produit.id !== null) {
                console.log("vm.produit.id = " + vm.produit.id + " vm.CatProduit.nom = " + vm.produit.catProduit.nomCatProduit);//pour le débogga, on peut le rétirer
                ProduitService.update({id: vm.produit.id}, vm.produit, onSaveSuccess, onSaveError);
            } else {
                vm.produit.datePublication = new Date();
        ...
        function loadAll() {//Fontion qui affiche la liste des catégories de produits
